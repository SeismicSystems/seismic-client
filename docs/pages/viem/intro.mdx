...
const wallet = createShieldedWalletClient({
  // [!code focus]
  chain: seismicDevnet1, // [!code focus]
...
You can create a Shielded Contract Instance with the `getShieldedContract` function by passing in a [ABI](https://viem.sh/docs/glossary/types#abi), address, and [Shielded Public](/docs/clients/public) and/or [Shielded Wallet Client](/docs/clients/wallet). Once created, you can call contract methods, fetch for events, listen to events, etc.
