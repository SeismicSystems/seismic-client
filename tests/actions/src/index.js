...
import { createShieldedPublicClient, createShieldedWalletClient, } from '@actions/client';
...
const testContractBytecodeFormatted = `0x${bytecode.object.replace(/^0x/, '')}`;
...
const textSeismicTx = async () => {
    ...
    const deployedContractAddress = await getDeployedAddress(publicClient, walletClient.account.address);
    ...
    console.info(`[1] setNumber receipt: ${JSON.stringify(receipt1, stringifyBigInt, 2)}`);
    ...
    console.info(`[2] Increment receipt: ${JSON.stringify(receipt2, stringifyBigInt, 2)}`);
    ...
    console.info(`[3] setNumber receipt: ${JSON.stringify(receipt3, stringifyBigInt, 2)}`);
